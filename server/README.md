...
If the body includes **`new_contact`** `{ name, email?, phone? }` instead of **`contact`** ids, the server creates a **Contact** row, links it to the selected **customer** (see `AIRTABLE_QUOTE_*` and `QUOTE_*_LINK_FIELD` in `server/.env.example`), then creates the quote with that contact id. Otherwise behavior is unchanged: creates a row in **Airtable** (`AIRTABLE_QUOTES_TABLE_ID`), resolves **`quote_reference`**, then **copies the quote Google Doc template** (`GOOGLE_DOCS_QUOTE_TEMPLATE_ID`), replaces placeholders (`{{doc_num}}`, `{{client}}`, `{{products}}`, `{{transporting}}`, `{{additionals}}`, prices, etc.), exports PDF to Drive, patches **מסמכים נוספים** (or `QUOTE_FIELD_QUOTE_DOCUMENT`), and if **שליחה ללקוח** is checked sends HTML email to the contact with the PDF attached. The reference is **not** written by the API. `GET /api/quote/next-reference` returns the same prediction for the form preview.
